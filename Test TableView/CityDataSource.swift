...
// The next line means that the class CityDataSource is extending NSOBject and UITableViewDataSource. This is neccessary for getting the UITableView to be able to extract and display this data.
class CityDataSource: NSObject, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let item:[String] = data[indexPath.row].components(separatedBy: ",") //the first item is "Seattle, WA"
        //HINT: Use the Developer Documentation window to serach for .components(separatedBy
        
        cell.city.text = item[0]
        cell.state.text = item[1]
